...
public class MainPregunta2 {
    public static void main(String a[]) {
        ...
        private static int cuentaEdadesImpares(LinearNode<T> node){
                int contador = 0;
                while(node.getSig() != null){
                        if(node.getDato instanceof Persona){
                                if(node.getDato().getEdad()% 2 != 0)
                                        contador++;
                        }
                        node = node.getSig();
                }
        ...
        private static void insertaEnLaPos(int i,LinearNode<T> node,LinearNode<T> nodes ){
                if(i<0)
                 return;
                if(i > Long(node)){
                        while(node.getSig() == null)
        ...
        private static int Long(LinearNode<T> node){
                int contador =0;
                while(node.getSig() != null){
                        contador++;
                        node = node.getSig();
